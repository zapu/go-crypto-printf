...
func TestImports(t *testing.T) {
	entities, err := ReadArmoredKeyRing(strings.NewReader(pub25519_kbpgp))
	if err != nil {
		t.Fatalf("error opening keys: %v", err)
	}
	if len(entities) != 1 {
...
func TestInvalidEddsaSignatureImport(t *testing.T) {
	...
	_, err := ReadArmoredKeyRing(strings.NewReader(pub25519_invalid_sig))
	if err == nil {
	...
	entities, err := ReadArmoredKeyRing(strings.NewReader(pub25519_invalid_sig2))
	if len(entities) != 1 {
...
const pub25519_kbpgp = `-----BEGIN PGP PUBLIC KEY BLOCK-----
...
const pub25519_invalid_sig = `-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: Keybase OpenPGP v2.0.58
Comment: https://keybase.io/crypto
...
const pub25519_invalid_sig2 = `-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: Keybase OpenPGP v2.0.58
Comment: https://keybase.io/crypto
